...
from bivariate_gaussian_quadrature import compute_bivariate_expectation
...
def compute_outer_correlation_matrix(mean_internal, cov_internal, transforms, n_points=25):
    """
    Compute correlation matrix between all pairs of transformed parameters using bivariate quadrature.
    ...
    transforms : list
        List of transformation functions (one per parameter, in same order as mean_internal)
    n_points : int
    ...
    -------
    ndarray
        Correlation matrix between transformed parameters
    """
    ...
    for i in range(n_dim):
        ...
        def transform_i_func(x, direction):
            return transform_func(x, direction)
        
        # Compute marginal statistics
        result = compute_variance_gauss_hermite(mu_i, var_i, transform_i_func, n_points)
    ...
    for i in range(n_dim):
        for j in range(n_dim):
            if i == j:
                ...
                print(f"  Computing Corr(X_{i+1}, X_{j+1})...", end=" ")
                ...
                transform_func_i = transforms[i]
                transform_func_j = transforms[j]
                ...
                # Correlation coefficient: Corr(X,Y) = (E[XY] - E[X]E[Y]) / (σ_X σ_Y)
                covariance_outer = cross_moment - mean_outer[i] * mean_outer[j]
                correlation_outer = (covariance_outer / np.sqrt(var_i_outer * var_j_outer)
                                   if var_i_outer * var_j_outer > 0 else 0.0)
                
                outer_corr_matrix[i, j] = correlation_outer
                outer_corr_matrix[j, i] = correlation_outer  # Symmetric
                
                print(f"{correlation_outer:.4f}")
            else:
                # Lower triangle - already filled by symmetry
                pass
    
    return outer_corr_matrix
...
def test_multivariate_transformation():
    ...
    # Step 4: Compute pairwise correlations in outer space
    print("4. Computing Pairwise Correlations in Outer Space")
    print("-" * 50)
    ...
    print("Internal (Gaussian) correlation matrix:")
    print(internal_corr_matrix)
    print()
    
    # Compute outer correlations using the new efficient function
    outer_corr_matrix = compute_outer_correlation_matrix(
        mean_internal, cov_internal, transforms, n_quad_points
    )
    
    print("\nOuter (Transformed) correlation matrix:")
    ...
    print(f"{'Parameter':<12} {'Transform':<25} {'Mean Change':<12} {'Var Change':<12} {'Corr Change':<12}")
    print("-" * 85)
    ...
    for i in range(n_dim):
        ...
        # Average correlation change for this parameter
        corr_changes = []
        for j in range(n_dim):
            if i != j:
                corr_changes.append(abs(outer_corr_matrix[i, j] - internal_corr_matrix[i, j]))
        avg_corr_change = np.mean(corr_changes) if corr_changes else 0.0
        
        transform_name = transforms[i].name.split(' ')[0]
        
        print(f"{i+1:<12} {transform_name:<25} {mean_change:<12.4f} {var_change:<12.4f} {avg_corr_change:<12.4f}")
    ...
    print("✓ Computed pairwise correlations using bivariate Gaussian quadrature")
