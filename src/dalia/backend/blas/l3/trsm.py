...
def trsm(
    ...
    """
    if hw_target == "host":
        return solve_triangular_host(
            a, b, trans, lower, unit_diagonal, overwrite_b, check_finite, side
        )
    elif hw_target == "accelerator":
        return solve_triangular_accelerator(
            a, b, trans, lower, unit_diagonal, overwrite_b, check_finite, side
        )
    else:
        raise ValueError(
...
def solve_triangular_accelerator(
    a,
    b,
    trans=0,
    lower=False,
    ...
    if batch_count:
        ...
        b = cp.array(b, dtype=dtype, order="F", copy=(None if overwrite_b else True))
...
def _solve_triangular(
    a1, b1, trans=0, lower=False, unit_diagonal=False, overwrite_b=False, side=0
):
    ...
    if a1.flags.f_contiguous or trans == 2:
        ...
        # transposed system is solved since trsm expects Fortran ordering
        x = trsm(
            alpha,
            a1.T,
            b1,
