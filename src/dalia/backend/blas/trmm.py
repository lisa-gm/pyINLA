...
def trmm(
    a, b, hw_target, alpha=1.0, side=0, lower=0, trans_a="N", diag=0, overwrite_b=0
):
    ...
    if hw_target == "host":
        ...
        return matmul_trmm_accelerator(
            a, b, alpha, trans_a, side, lower, diag, overwrite_b
        )
    else:
        ModuleNotFoundError("Unknown Module")


def matmul_trmm_host(
    a,
    b,
    alpha=1.0,
    trans_a=0,
