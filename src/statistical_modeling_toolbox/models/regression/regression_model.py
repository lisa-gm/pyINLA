...
class ModelName(BaseModel):
    ...
    def build(self, data: Dict[str, Any], formula: Optional[str] = None) -> "ModelName":
        """
    ...
    def precision_matrix(
        self, hyperparams: Dict[str, Any], sparse_format: str = "csr"
    ) -> sparse.spmatrix:
        """
        ...
        if not self._is_built:
            raise RuntimeError(
                "Model must be built before computing precision matrix. Call build() first."
            )
    ...
    def log_density(self, x: NDArray, params: Dict[str, Any]) -> float:
        """
    ...
    def gradient(
        self, x: NDArray, params: Dict[str, Any], wrt: str = "x"
    ...
    def sample(
        self, n_samples: int, params: Dict[str, Any], random_state: Optional[int] = None
    ) -> NDArray:
        """
...
def _construct_difference_operator(n: int, order: int = 1) -> sparse.spmatrix:
    ...
    if order == 1:
        ...
        raise NotImplementedError(
            f"Difference operator of order {order} not implemented"
        )
...
def validate_precision_matrix_properties(
    Q: sparse.spmatrix, tol: float = 1e-10
) -> Dict[str, bool]:
    """
