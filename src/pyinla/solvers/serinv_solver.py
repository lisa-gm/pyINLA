...
class SerinvSolverFullGPU(Solver):
    """Serinv Solver class."""
    ...
    def __init__(
        ...
        # Print the allocated memory for the BTA-array
        total_bytes = (
            self.A_diagonal_blocks_d.nbytes
            + self.A_lower_diagonal_blocks_d.nbytes
            + self.A_arrow_bottom_blocks_d.nbytes
            + self.A_arrow_tip_block_d.nbytes
        )
        total_gb = total_bytes / (1024**3)
        print_mpi(f"Allocated memory for BTA-array: {total_gb:.2f} GB", flush=True)
    ...
    def _sparray_to_structured(self, A: sparray, sparsity: str) -> None:
        ...
        for i in range(self.n_diagonal_blocks):
            ...
            self.A_diagonal_blocks_d[i, :, :] = csr_slice.todense()
            ...
            if i < self.n_diagonal_blocks - 1:
                ...
                self.A_lower_diagonal_blocks_d[i, :, :] = csr_slice.todense()
            ...
            if sparsity == "bta":
                ...
                self.A_arrow_bottom_blocks_d[i, :, :] = csr_slice.todense()

        if sparsity == "bta":
            csr_slice = A_csr[-self.arrowhead_blocksize :, -self.arrowhead_blocksize :]

            self.A_arrow_tip_block_d[:, :] = csr_slice.todense()
