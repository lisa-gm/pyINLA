...
class HyperparameterManager:
    ...
    def __init__(self, configs: List[HyperparameterConfig], track_history: bool = True):
        ...
        self._history_array: Optional[np.ndarray] = (
            None  # Dense 2D array (iterations x params)
        )
    ...
    def __getitem__(self, name: str) -> float:
        ...
        if name not in self._values:
            raise KeyError(
                f"Hyperparameter '{name}' not found. Available: {list(self._values.keys())}"
            )
    ...
    def update_from_array(
        self, array: np.ndarray, iteration: Optional[int] = None, note: str = ""
    ) -> None:
        """
    ...
    def _append_to_history(self, iteration: int, note: str = "") -> None:
        ...
        entry = {
            ...
            "note": note,
            "array": (
                self._last_update_array.copy()
                if self._last_update_array is not None
                else None
            ),
        }
    ...
    def get_history_array(self) -> np.ndarray:
        ...
        if not self._track_history:
            raise ValueError("History tracking is disabled")
        return (
            self._history_array.copy()
            if self._history_array is not None
            else np.empty((0, len(self)))
        )
    ...
    def get_historical_array(self, iteration: int) -> np.ndarray:
        ...
        if iteration < 0 or iteration >= len(self._history):
            raise IndexError(
                f"Iteration {iteration} out of range (0 to {len(self._history)-1})"
            )
        ...
        if self._history_array is not None and iteration < len(self._history_array):
            ...
            return np.array(
                [self._history[iteration]["values"][name] for name in self._param_names]
            )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "HyperparameterManager":
        ...
        if (
            data.get("track_history", True)
            and "history" in data
            and data["history"] is not None
        ):
            ...
            if manager._history:
                ...
                for entry in manager._history:
                    if "array" in entry and entry["array"] is not None:
                        ...
                        # Fallback: reconstruct from values dict
                        values = [
                            entry["values"][name] for name in manager._param_names
                        ]
    ...
    def __repr__(self) -> str:
        ...
        values_str = ", ".join(
            [f"{name}={self._values[name]:.6g}" for name in self._param_names]
        )
        history_info = (
            f", {self._iteration_counter} iterations" if self._track_history else ""
        )
...
def example_usage():
    ...
    def objective(x):
        ...
        # Print progress
        print(
            f"Iter {hp.get_last_iteration()-1}: loss={loss:.6f}, lr={lr:.6f}, bs={bs:.1f}, l2={l2:.2e}"
        )
        return loss
    ...
    result = minimize(
        objective, x0, method="L-BFGS-B", options={"maxiter": 20, "disp": False}
    )
