...
#SBATCH --output=output_gaussian_spatioTemporal_bench_allGPU.out
#SBATCH --error=output_gaussian_spatioTemporal_bench_allGPU.err
#####SBATCH -C gpu
...
export NSYS=1
export NSYS_FILE=gaussian_spatioTemporal_bench_pinned_numaCtl_allGPU_omp${OMP_NUM_THREADS}_numRanks${num_ranks}
...
set -x
srun -n ${num_ranks} ./bind.sh ./nsys.sh python run.py >output_gaussian_spatioTemporal_bench_pinned_numaCtl_allGPU_omp${OMP_NUM_THREADS}_numRanks${num_ranks}.txt
