...
if __name__ == "__main__":
    print_msg(
        "--- Example: Federated Model Regression (optional: site specific intercept) ---"
    )
    ...
    prec_o_idx = list(federated_model.theta_keys).index("prec_o")
    ...
    df_dalia.to_csv(
        f"{BASE_DIR}/dalia_summary_{data_type}_{intercept_tag}.csv",
        index=False,
    )
    print_msg(df_dalia)
