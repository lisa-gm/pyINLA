...
if __name__ == "__main__":
    ...
    for i in range(n_replicates):
        ...
        ar1_dict = {
            "type": "ar1",
            "input_dir": f"{BASE_DIR}/old_inputs/replicate_{i+1}/inputs_ar1",
        ...
        regression_dict = {
            "type": "regression",
            "input_dir": f"{BASE_DIR}/old_inputs/replicate_{i+1}/inputs_regression",
            "n_fixed_effects": 1,
    ...
    # save Qprior to file
    sp.sparse.save_npz(f"{BASE_DIR}/old_inputs/reference_outputs/Qprior.npz", Qprior)
    Qconditional = replicate_model.construct_Q_conditional(
        eta=replicate_model.a @ replicate_model.x
    )
    sp.sparse.save_npz(f"{BASE_DIR}/old_inputs/reference_outputs/Qconditional.npz", Qconditional)
    A = replicate_model.a
    sp.sparse.save_npz(f"{BASE_DIR}/old_inputs/reference_outputs/A.npz", A)
    ...
    theta_ref = xp.load(f"{BASE_DIR}/old_inputs/replicate_1/reference_outputs/theta_original.npy")
    x_ref = xp.load(f"{BASE_DIR}/old_inputs/replicate_1/reference_outputs/x_ref.npy")
    ...
    if save_dalia_results:
        save_to_json(
            results=results,
            filename=f"{BASE_DIR}/old_inputs/reference_outputs/dalia_estimates.json",
        )
